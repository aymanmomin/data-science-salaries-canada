...
# Title
st.markdown('<p class="main-header">🇨🇦 Data Science Salary Explorer</p>',
            unsafe_allow_html=True)
st.markdown('<p class="sub-header">Interactive analysis for students planning co-op and career decisions</p>',
            unsafe_allow_html=True)
...
def load_data():
    ...
    df['Adjusted_Median'] = (df['Median_Salary'] /
                             df['Cost_of_Living_Index']) * 100
    df['Adjusted_Low'] = (df['Low_Salary'] / df['Cost_of_Living_Index']) * 100
    df['Adjusted_High'] = (df['High_Salary'] /
                           df['Cost_of_Living_Index']) * 100
    df['Salary_Range'] = df['High_Salary'] - df['Low_Salary']
    df['Growth_Potential'] = (
        (df['High_Salary'] - df['Low_Salary']) / df['Low_Salary'] * 100).round(1)
...
if len(filtered_df) == 0:
    ...
    with col_right:
        ...
        ax2.set_title('Entry-Level to Senior Range',
                      fontsize=14, fontweight='bold')
    ...
    st.dataframe(
        display_df.style.background_gradient(
            subset=['Median ($k)', 'Adjusted ($k)'], cmap='RdYlGn')
        .format({
            'Median ($k)': '{:.0f}',
            'Entry ($k)': '{:.0f}',
            'Senior ($k)': '{:.0f}',
